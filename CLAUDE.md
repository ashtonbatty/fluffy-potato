...
# Override role variables at runtime (note: appname_role_ prefix required)
ansible-playbook -i inventory/hosts orchestrate.yml -e service_action=stop -e "appname_role_post_kill_wait_seconds=10"
ansible-playbook -i inventory/hosts orchestrate.yml -e service_action=stop -e "appname_role_stop_expect_zero_rc=false"

# Disable force kill for safety
ansible-playbook -i inventory/hosts orchestrate.yml -e service_action=stop -e "appname_role_allow_force_kill=false"
...
Central role in `roles/appname_role/` that handles all service operations:
...
Each service has a vars file in `vars/` with appname_role_ prefixed variables:
- `appname_role_service_name` - Service identifier
- `appname_role_service_script` - Path to control script
- `appname_role_process_identifier` - Pattern for pkill fallback (must be >= 5 chars, properly quoted for security)
...
- **Security Controls**:
  - `appname_role_allow_force_kill` - Explicit boolean to enable/disable pkill fallback (default: true)
  - `appname_role_process_identifier` - Validated for length >= 5 chars to ensure specificity
...
- `appname_role_*_check_rc: false` - Disable RC validation entirely
- `appname_role_*_expect_zero_rc: false` - Invert logic (non-zero = success)

Applies to: `appname_role_start_check_rc`, `appname_role_stop_check_rc`, `appname_role_status_check_rc`

## Adding a New Service

1. Create `vars/newservice.yml` with service configuration (using `appname_role_` prefix):
   ```yaml
   appname_role_service_name: "newservice"
   appname_role_service_script: "/scripts/newservice.sh"
   appname_role_process_identifier: "COMPONENT=newservice"
